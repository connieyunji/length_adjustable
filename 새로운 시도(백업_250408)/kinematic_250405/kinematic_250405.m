...
for i = 1:length(vars)
    assignin('base', vars{i}, values(i)); % 워크스페이스에 변수 저장
end

%% ROM 계산 (4,1,7,11,14,17)
% 기준 원점 (4번 포인트)
position_4 = [0, 0];
...
% 위치 계산 (연결된 링크들) 11,14,17,16,12,9,8
...
% total_theta도 1D 벡터로 변환
total_theta = (T1 + T2 + T3);
total_theta = total_theta(:);

% 위치 계산 (역으로 연결된 링크들) 13, 10, 5, 6, 3
...
%% 특정 영역 내부 포함 여부 확인 (삼각형 5-8-7-4 및 9-8-11-7)
...
% % 링크 위치를 점으로 표시
plot(position_4_x, position_4_y, 'ro', 'MarkerSize', 4, 'LineWidth', 2); % position_4 (怨좎젙 ?썝?젏)
...
plot(position_17_x_fix, position_17_y_fix, 'mo', 'MarkerSize', 2, 'LineWidth', 2); % position_17 (?넀?걹)
